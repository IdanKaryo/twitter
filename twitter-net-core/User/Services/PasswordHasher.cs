...
namespace Twitter.User.Services
{
    public sealed class PasswordHasher: IPasswordHasher
    {
        ...
        public HashedPassword HashPassword(string password, string saltP = null)
        {
            if (password == null)
                throw new ArgumentNullException(nameof(password));

            var saltAsBytes = saltP != null ? Encoding.UTF8.GetBytes(saltP) : GenerateSalt(SaltSize);
            var hash = HashPasswordWithSalt(password, saltAsBytes);

            var inArray = new byte[1 + SaltSize + hash.Length];
            inArray[0] = Version;
            Buffer.BlockCopy(saltAsBytes, 0, inArray, 1, SaltSize);
        ...
        private byte[] HashPasswordWithSalt(string password, byte[] salt)
        {
            ...
            using (var hashAlgorithm = HashAlgorithm.Create(HashAlgorithmName.Name))
            {
                ...
                {
                    throw new Exception($"Invalid hash algorithm {HashAlgorithmName.Name}");
                }
